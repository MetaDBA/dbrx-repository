...
select 'Sample interval was ' || round(avg(sysdate - a.sample_date)*86400) || ' seconds. CPU used was ' 
	...
	from system_event_snap_date1 a, v$sysstat b
...
from   v$system_event b,
       system_event_snap1 a
where  b.event = a.event
  and  b.total_waits > nvl(a.total_waits,0)
  and  100*((b.time_waited-a.time_waited)/(&tot_time_waited+.000001-&old_tot_time_waited)) > 0.5
...
drop table system_event_snap1 purge;
drop table system_event_snap_date1 purge;
create table system_event_snap1 as select * from v$system_event;
create table system_event_snap_date1 (sample_date,cpu)
