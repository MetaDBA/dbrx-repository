...
create table dba_locks_temp (inst_id, session_id, lock_type, mode_held, mode_requested, lock_id1, lock_id2)
as select inst_id, sid, type, lmode, request, id1, id2
from gv$lock;
...
insert into lock_holders 
  select w.inst_id,
         ...
         process,
         osuser
  from gv$session, dba_objects id1, dba_objects id2,
       dba_locks_temp w, dba_locks_temp h
 where h.mode_held      !=  0
  and  h.mode_held      !=  1
  and  w.mode_requested !=  0
  and  w.lock_type       =  h.lock_type
  and  w.lock_id1        =  h.lock_id1
  and  w.lock_id2        =  h.lock_id2
  and  id1.object_id (+) =  w.lock_id1
  and  id2.object_id (+) =  w.lock_id2
  and  gv$session.sid     =  w.session_id
  and  gv$session.inst_id =  w.inst_id;
...
insert into lock_holders 
  select h_inst_id, holding_session, null, null, 'None', null, null,
         lock_id1, null, null, null, null, null
    from lock_holders 
 minus
  select w_inst_id, waiting_session, null, null, 'None', null, null,
         lock_id1, null, null, null, null, null
    from lock_holders;

update lock_holders
  set ( mode_held, object_name1, pid, osuser ) =
    ( select gv$lock.lmode,
             dba_objects.object_name,
             gv$session.process,
             gv$session.osuser
         from dba_objects, gv$lock, gv$session
        where gv$lock.sid          = gv$session.sid
	  and gv$lock.inst_id      = gv$session.inst_id
          and gv$lock.id1     = lock_holders.lock_id1
          and dba_objects.object_id (+) = gv$lock.id1
          and gv$session.sid             = lock_holders.waiting_session 
          and gv$session.inst_id         = lock_holders.w_inst_id )
...
select  lpad(' ',2*(level-1)) || w_inst_id || ':' || waiting_session "Session",
...
connect by  prior waiting_session =
                    DECODE( holding_session,
                            ...
                            holding_session )
        and prior w_inst_id = DECODE (h_inst_id, w_inst_id, null, h_inst_id)
